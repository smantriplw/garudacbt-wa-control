...
export const stockTradeCommand = async (client: Client) => {
	client.command(
		...
		async (ctx) => {
			...
			const labels = data.Returns.map((ret) => ret.Date).map((x) =>
			...
			const averageData = years.map((year) => {
				const datas = data.Returns.filter(
					(ret) => dayjs(ret.Date).format('YYYY') === year,
				).map((ret) => ret.Close);
				return `- ${year} (${datas.reduce((a, b, _, d) => a + b / d.length, 0).toFixed(2)})`;
			});

			const m = await ctx.reply(
				`Generating chart for *${data.Name}* (${data.Code})\n${labels.length} data`,
			);
					...
					options: {
						...
						scales: {
							...
							y: {
								...
								min:
									data.Returns.sort((a, b) => a.Open - b.Open)[0].Open < 600
										? 0
										: data.Returns.sort((a, b) => a.Open - b.Open)[0].Open -
											500,
								beginAtZero:
									data.Returns.sort((a, b) => a.Open - b.Open)[0].Open <= 500,
								ticks: {
									display: true,
									align: 'center',
									autoSkip: false,
